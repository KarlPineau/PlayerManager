...
$(document).ready(function() {
    ...
    function addBAB(container, index) {
    ...
    $.each($('#dndrules_classdndbundle_classDnD_register_levels').children(), function() {
        ...
        var generateBAB = $('#'+id).children().last().html();
    ...
    $.each($('div[id$="_classBABs"]'), function() {
    ...
    $.each($('.add-line'), function() {
        $(this).on('click', function() {
            if($('#'+$(this).attr('attr-id')+'_classBABs').children().length > 0) {
                var idclassBABs = $('#' + $(this).attr('attr-id') + '_classBABs').children().last().children().first().children().last().children().first().attr('id');
                var intToIncre = parseInt(idclassBABs.substr(idclassBABs.length - 1));
            } else { intToIncre = -1; }

            addBAB($('#'+$(this).attr('attr-id')+'_classBABs'), (intToIncre+1));
        });
    });
